...
public:
    ...
    * 调用方负责保证同一时刻只有一个未决请求。
    *
    * **调用方不止一个**: 扫描状态机 (ScanController, 一整趟里几千次) 与界面上那个
    * 「读一次」按钮 (人工点)。所以这条约定不是"每个调用方自己跟自己保证", 而是**它们
    * 之间要协调**的 —— 目前靠 ScanWindow 里那条闸: 控制器不在 Idle 时按钮禁用
    * (见 scanwindow.cpp 的 onReadOnceClicked)。再加第三个调用方时, 那条闸得一起看:
    * 违约的表现不是报错, 而是**两个数分给两边** (模拟源) 或者**一边白等到超时**
    * (真机按时间戳只认严格更新的采样) —— 都是默默发生的。 */
