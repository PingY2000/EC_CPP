...
> **2026-09-17 补**:这个量程现在是一个**运行期的值**而不是编译期常量了 ——
...
- CLI 未被碰坏:`motor_test.exe` 不带任何 `--allow*` 跑通,**退出码 0**,
  `6064h = 0`、`6502h = 0xA5`。
  > **这里原先写的是当次读到的 `1C12h→1600h`(5 项)/ `1A00h`(5 项),已删 ——**
  > 它与后来 [ykd2205pe_ci402.md](ykd2205pe_ci402.md) 记下的
  > **`1C12h→1601h`(7 项)/ `1A00h`(3 项)** 对不上,而两台记录都是真的:
  > 这台驱动器的 RxPDO 映射**被改过**(很可能是厂家上位机),同一块板子在不同日期
  > 读到不同映射是常态。所以**别在文档里把映射读数当固定事实写死** ——
  > 每次都实读,实读值以 `motor_test` 跑出来的那份为准。
