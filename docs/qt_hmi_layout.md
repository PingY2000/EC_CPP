...
- `source` 列写这条事实来自哪一版（`V1.0` / `V1.1` / `V2.3` / `V2.4` / `ESI` / `实测`）。
  把它变成数据列，以后 GUI 里可以直接显示"这条的依据是 V2.4 YKD 附录，与 ESI 声明不一致"。
...
[ykd2205pe_ci402.md](ykd2205pe_ci402.md) 更正的核心是
...
> 后并未按标准迁到 Switch on disabled（仍停在 bit5=1/bit6=0）。详见文档「状态字 6041h」。

---

## 8. 过程数据镜像面板 (`panel_pdo_mirror`)
...
实测输出镜像 **23 字节**是奇数，与手册（2 B）、ESI 声明（16 B）、
ESI 字典默认（3 B）**都对不上**。唯一可信的是实读映射，
所以这个"只能实读、不能假设"的结论**做成界面**。
...
- 用的是 **MSYS2 UCRT64 的 MinGW + `hmi-qt-ucrt64` preset（Ninja）**，
  `wpcap.lib`/`Packet.lib` 由 MinGW 直接链接；真正的约束是
  **界面用的 Qt 必须与 SOEM 同一套 CRT**
  （见 §0 末段与 [hmi_click_position.md](hmi_click_position.md) §6.1）。
  用 `qt_standard_project_setup()` + `qt_add_executable`。
