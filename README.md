...
- `scan` (Qt 上位机, 自动采集) 与 `hmi` **同一套护栏**, 但它是**一段没人看着的长动作**
  ...
  - 布局: 左画布 | 右参数栏, 中间一根**可以拖的分隔条**。窗口变大, 多出来的全给画布;
    参数栏自己上下滚 (五组框约 1240px, 窗口通常只给它 670)。
  - 「扫描参数」组底部有**「恢复默认」**: 这一组每一项都回到缺省值
    (区域 27×27 / 分辨率 0.5 / 1 单位 = 50000 脉冲 / 速度 20000 pul/s …), **CSV 路径不动**。
  - **硬件限位 (`6041h` bit11) 三处同时显示**: 状态栏常显的两根轴 (**信号灯 + 文字**)、
    画布上实测位置点外面的一圈红环 + 是哪根轴、以及上升沿那条不自动消失的横幅。
  ...
  - **参数栏「轴信号」组** (排在最上面): 每根轴三个信号、一个信号一盏灯 ——
    **使能 / 故障 / 限位**, 右边跟着各自那行字 (已使能/未使能、无故障/有故障、正常/撞上)。
    它排在最前是刻意的: 参数栏比窗口高是常态, 而**要滚才能看到的状态指示器不算状态指示器**。
