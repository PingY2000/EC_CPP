...
- `scan` (Qt 上位机, 自动采集) 与 `hmi` **同一套护栏**, 但它是**一段没人看着的长动作**
  ...
  - **「X/Y 正/反向回零」** 与 **「X/Y 找正/找负限位」** (参数栏第三块, 两排共八个按钮,
    上面那排找**原点开关** `X0`, 下面那排把**限位开关本身**当机械基准 —— 两排走的是
    **同一条** `em_home()`, 共用下面那个回零速度):
    ...
    **回零用的是驱动器自己读到的开关电平, 所以「高级选项」里的「上位机侧取反」对它无效**;
    ...
    - **上面那排是找原点开关**, 方式 **24** (正向) / **29** (反向); **下面那排是找限位**,
