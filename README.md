...
- **参数会记住。** 关窗 (以及每次「连接」) 时写 **exe 旁边的 `bin/scan.ini`** ——
  ...
  「让 60FDh 进 TxPDO」都不记** (见 [docs/scan_sweep.md](docs/scan_sweep.md))。
...
- `scan` (Qt 上位机, 自动采集) 与 `hmi` **同一套护栏**, 但它是**一段没人看着的长动作**
  ...
  - **「X/Y 正/反向回零」** (参数栏第三块, 驱动器自带的 HM 模式, 方式 24 / 29):
    ...
    **「回零速度」输入框**喂 `6099h:01`; 返回速度与加减速都由它派生 (返回 = 1/4,
    ...
    下限 100。**不进 `scan.ini`** (回零框里那行字会把当次的数念一遍)。
    超时 30 秒 —— 它同时是一条**距离上限** (速度 × 时间): 缺省速度下是 30 圈, 够走完
    缺省区域的一头到另一头; 调低会跟着缩小。
    **回零用的是驱动器自己读到的开关电平, 所以界面上那个「输入电平反转」对它无效**;
    极性配反时 **必须先去改驱动器的 `2300h`**。
