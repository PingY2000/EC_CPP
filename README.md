...
- `scan` (Qt 上位机, 自动采集) 与 `hmi` **同一套护栏**, 但它是**一段没人看着的长动作**
  ...
  - **限位判据 (默认 = `6041h` bit11「硬件限位信号有效」) 几处同时显示**: 状态栏常显的
    ...
    那个红**故意不复用"采失败"的红** —— 一个是数据问题, 一个是限位信号报有效。
  ...
  - **「故障复位」**清驱动器的故障位 (`6040h` bit7 上升沿)。**只对报了故障的轴写字节**
    ...
    **不弹确认框** —— 按下去就发, 但"它看起来要复位哪根轴"会用横幅说出来;
    每轴最多 1 秒且**不可中断** (按钮在这期间显示「正在复位…」)。
  - **「X/Y 正/反向回零」** (参数栏第三块, 驱动器自带的 HM 模式, 方式 24 / 29):
    ...
    **不弹确认框** —— 事前的说明在按钮自己的 tooltip 上 (轴 / 方向 / 方式号 /
    ...
    写在回零框里那行**随速度实时变**的字上 (它从前是那个模态的内容, 模态去掉后
    这些数不能没有去处 —— "够不着就把滑台挪近、**别去调高速度**"正是防硬顶的那一句)。
    回零期间**该轴按钮变「回零中…」**、顶部出横幅, 而「使能 / 失能 / 停止点 /
    设为区域中心 / 开始扫描」全部锁住。结束后**自动切回 CSP 并保持使能**
    (停在落点带保持力矩), 并**从 `6064h` 重新锚定显示零点** —— 不锚定就是一次
    没人按过按钮的全速运动。**「回零速度」输入框**喂 `6099h:01`; 返回速度与加减速都由
    它派生 (返回 = 1/4, 加减速按**斜坡时间 0.1 秒**算 —— 写死一个加速度的话, 速度一放开
    就成了 20 秒斜坡, 把速度调上去反而看不出区别)。**上限与「扫描速度」同一个数
    (100000 pul/s ≈ 2 圈/秒), 缺省 50000 是驱动器自己 `6099h:01` 的实测值**;
    下限 100 留给第一次试方向。**不进 `scan.ini`** (回零框里那行字会把当次的数念一遍)。
    超时 30 秒 —— 它同时是一条**距离上限** (速度 × 时间): 缺省速度下是 30 圈, 够走完
    缺省区域的一头到另一头; 调低会跟着缩小。找不到开关不会一直找下去。
    ⚠️ **回零用的是驱动器自己读到的开关电平, 所以界面上那个「输入电平反转」对它无效**;
    极性配反时 (2026-09-18 那台机器就是) **必须先去改驱动器的 `2300h`**。
  - **「停止」在回零进行中 = 立即中止** (≤2ms 被 `em_home()` 的轮询看见),
    **并且保持使能** —— 中途位置不明, 卸力会让竖直轴自由下滑。
    ...
    安全理由是**结构性**的 (那行只往一个原子量里存 1, 不碰总线、不做 I/O),
    理由写在 [hmi/ecatworker.h](hmi/ecatworker.h) 顶部。关闭窗口 / 断开时也会先中止回零。
  - **自动中止** (红色横幅 + 模态框): 掉出 OP / `6041h` bit3 故障 /
    **`6041h` bit11「硬件限位信号有效」** (勾了输入反转则改看反相后的正/负限位开关,
    见 `ecatcmd::limit_rule_for`) / WKC 连续掉帧 / 丢帧或掉使能 / 有人从旁路改了目标 /
    走到位超时 / CSV 写失败。bit11 这一条是**最可能真触发**的那一条, 不是凑数的。
    注意 bit11 报的是那路限位信号**此刻的电平** (手册原话), 不是"撞过了"也不是闩锁 ——
    回零时它本来就该是 1, 而**极性配反**会让它恒为 1 (2026-09-18 那台机器就是)。
  ...
  - **「读一次」按钮**: 向当前取样源要一个数并显示, **不用跑整趟扫描**就能确认接上了、
    读数合理 (刚插上表头时最有用的一个动作)。它走的是扫描用的同一条路, 并显示**一次
    往返的实测毫秒数** —— 那个数必须远小于扫描参数里的读数超时。
    **扫描进行中它是禁用的**: 接口约定"同一时刻只允许一个未决请求" (`powermeter.h`),
    而那个请求归扫描状态机; 抢同一个数的后果是 CSV 里**悄悄少一个点或错一个点**。
  - 其余同 `hmi`: 启动后一个字节都不写; 收尾顺序是「先中止扫描」→ 失能 → 还原 PDO 映射
    → 降 `PRE_OP` → 关网卡; 收尾未能确认失能时**模态告警** (= 退出码 **10** 的语义) ——
    见到请立即断掉驱动器动力电源。
    ⚠️ **动作前的确认框在本程序里被全部去掉了** (「连接」「使能」「故障复位」「回零」
    「开始扫描」「续扫」「关窗」) —— 按下去就执行。"会发生什么"改由三样东西在**按之前**
