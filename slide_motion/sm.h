 ...
 * 阶段: S0 身份/总线门禁(只读) | S1 参数基线与漂移(只读, 外部基线文件)
 *       S2 OP循环+DC(本版未实现) | S3 使能状态机(写 6040h/6060h, 通电不移动)
 *       S4 微动+反馈闭环(写 6040h/607Ah/6081h/6083h/6084h, 会动) | S5 收尾/安全停机
 * 安全边界: 全工程只有 sm_guard.c 出现 ecx_SDOwrite, 其余经 sm_wr_* 与 sm_set_cw() 写。
 *   默认不动 (需 --allow-motion; 移动还需 --allow-jog); 上限只能收紧, 放宽需
 *   --force-caps 且非交互确认; 任何异常路径 (含 Ctrl-C/故障/丢站/超时) 必须走
 *   sm_guard_teardown(), 让 6040h 回到失能态。
 */
...
#define SM_EXIT_NOT_DISABLED 10 /* 收尾写 6040h=0 后 6041h 仍报 Operation enabled:
                                   电机可能仍带电; 最严重的失败, 覆盖其它码 */
...
/* 受支持的 AL 状态与 SDO 超时。
 * SOEM 的 EC_TIMEOUTRXM = 700000us = 700ms, 运动期太长, 改用
 * SM_SDO_TMO_MOTION (200ms), 使最坏反应时间约 250ms。 */
...
/* 心跳看门狗必须 > 单次轮询最坏耗时 (3 次 SDO 读 × SM_SDO_TMO_MOTION = 600ms);
 * 该不等式由 sm_guard_check_caps() 强制。 */
...
/* --force-caps 能放宽到的绝对天花板。防止 |INT32_MIN| 取绝对值溢出骗过比较,
 * 传出 2^31 脉冲。100000 = 硬上限 (2000) 的 50 倍。 */
#define SM_CAP_ABS_MAX        100000L

/* 容差上限 (脉冲): 容差放到比行程还大即等于删掉 A1/A6 动作检查。 */
...
/* CiA402 控制字 (6040h)。
 * 6041h 低 4 位是非标准编码 0000/0001/0011/0111, 全程用位判断, 禁止数值比较。 */
 ...
 * 只对文档明确列出的位置断言; 其余 (bit4/bit7/bit9/bit11) 只记录不断言。 */
...
/* 数据类型提示。仅用于决定符号性; 实际字节宽度一律以驱动器 SDO 返回的
 * psize 为准 (手册没给类型的对象不猜, 靠驱动器自报)。 */
#define SM_DT_U8  0
#define SM_DT_I8  1
#define SM_DT_U16 2
#define SM_DT_I16 3
...
typedef struct
{
   ...
   /* sw 是否为一次真实读数: 0x0000 是合法的 "Not ready to switch on",
    * 不能当"没读到"的哨兵; sw_valid=0 时报告与 CSV 必须打 "----"。 */
   char     sw_valid;
...
typedef struct
{
   ...
   char fail;
   char warn;
   /* 本次运行是否接管过这根轴 (已过前置检查、准备使能/微动)。
    * 收尾与急停只处理 engaged 的轴 —— 别的轴可能正被另一套程序控制。 */
   char engaged;
...
typedef struct
{
   ...
   /* 收尾写完 6040h=0 后 6041h 仍报 Operation enabled, 即电机可能仍带电;
    * 本次运行最严重的失败, 退出码与汇总都要反映。 */
...
/* 把刚读到的 6041h 回填进最近一条轨迹记录。valid=0 表示没读到:
 * 不写 sw, 该条保持 sw_valid=0, 报告与 CSV 打 "----"。 */
void sm_trace_fill_sw(sm_axis_t *ax, uint16_t sw, int valid);

/* 急停 (尽力而为) 与收尾 (幂等, 含所有错误路径)。绕过授权检查直接写,
 * 否则 Ctrl-C 之后反而停不下来; 只写去使能方向的 6040h, 永不使能。 */
...
/* dead-man 检查: 距上一次成功总线交互已超过 watchdog_ms。
 * 本版 SOEM 的 slavelist[].islost 从不被置位, 掉线只表现为 SDO 一直超时。
 * 返回非 0 = 已超时, 调用者应当中止。 */
 ...
 * 每次必须先清 ctx.ecaterror 粘滞位, 否则上次失败污染本次判定。 */
...
/* SDO 事务日志 (实现见 sm_bus.c)。归并只对读: 签名 (slave, index, sub, rc, abort) 相同的
 * 连续读折叠成一行, 签名一变即落一行。写不归并 (0x0006/0x0007/0x000F 必须逐笔可见)。 */
/* 运动窗口开关。关的时候会先把未收尾的一行打出来。 */
void sm_xfer_set_active(int on);
/* 记一笔 SDO 事务。size/bytes 是驱动器自报的字节数与原始字节;
 * 读失败时传 size=0, bytes=NULL。 */
