 ...
 * 把现场实测值导出为外部基线文件 (--dump-baseline), 人工审定后作为验收基线;
 * 此后用 --baseline 与它比对并报告漂移。本文件不含任何写操作 (纯只读 SDO)。
 */
...
/* 参数规格表 —— 加对象只改这张表, 主循环不用动。字节宽度按驱动器自报的 psize,
 * 手册未给类型的对象也能读。dangerous=1: 该项漂移则 S3/S4 动作结论不可信, 禁止动作
 * (2310h~2313h 决定 60FDh 每位含义; 2201h/2400h/2408h/2409h 决定脉冲与位移换算)。 */
static const sm_spec_t sm_spec_table[] = {
   ...
   /* 不要往表里加 6041h: 它是状态字随使能状态一直在变, 且 dangerous=1 会让
      基线每次比对必然 FAIL 并拒绝 S3/S4。 */

   /* 行程当量: 决定"1 个脉冲 = 多少位移", 漂移即禁止动作。
    * 2201h=0 (出厂默认) -> 2400h 细分 (本机实测 50000 脉冲/圈); 2201h=1 -> 2408h/2409h
    * 电子齿轮比 (两套差 50 倍)。四个都留表里, 宽度 U16 (实机自报 2 字节)。 */
   { 0x2201, 0x00, SM_DT_U16, SM_CMP_EXACT, 1, 0, "2201h 当量选择 (0=细分/1=电子齿轮)" },
   { 0x2400, 0x00, SM_DT_U16, SM_CMP_EXACT, 1, 0, "2400h 细分: 脉冲/圈 (2201h=0 时生效)" },
   { 0x2408, 0x00, SM_DT_U16, SM_CMP_EXACT, 1, 0, "2408h 电子齿轮分子 (2201h=1 时生效)" },
   { 0x2409, 0x00, SM_DT_U16, SM_CMP_EXACT, 1, 0, "2409h 电子齿轮分母 (2201h=1 时生效)" },
...
/* 编译期断言: 表长须 <= SM_SPEC_COUNT。sm_base_load 拿表下标直接索引 slot,
 * 超出即静默越界写。加对象撞上这条就调大 sm.h 的 SM_SPEC_COUNT。 */
typedef char sm_spec_count_fits[SM_SPEC_N <= SM_SPEC_COUNT ? 1 : -1];
...
static int parse_key(const char *key, uint16_t *index, uint8_t *sub)
{
   ...
   *sub = 0;
   ...
   /* 索引一律按十六进制; "0x2310" 这种 base 16 也能吃下 */
...
/* 极简 INI 解析: # 或 ; 起注释; [section] 切节; key = value; value 十进制或 0x 十六进制。
 * 行首 '!' (在 [axis.N] 节内) = 必需项, 不匹配判 FAIL; 未知节/未知键 -> 计数并 WARN。 */
...
/* 把现场实测值导出为基线文件。全部项都不带 '!', 由人审定后再升级必需项;
 * 不可读的对象写成注释行, 不静默省略。 */
int sm_base_dump(const char *path, const sm_axis_t *axes, int nslaves)
{
   ...
   fprintf(f, "#   2310h~2313h (限位/原点输入功能定义) 与 2201h/2400h/2408h/2409h (行程当量)。\n");
   fprintf(f, "# 另有一档由代码决定 (sm_spec_table 的 dangerous 列): 那几项**不管有没有\n");
   fprintf(f, "# '!'**, 一旦漂移就拒绝执行使能与微动 (S3/S4)。\n");
   fprintf(f, "\n[meta]\n");
...
/* 无基线时打印当前实测值表 (只读, 不做 PASS/FAIL 判定)。
 * 不可读的项标出来 (对象不存在 vs 超时)。
 * 与 slide_verify 的对象表互为独立实现, 可互相印证读数是否一致。 */
...
/* 逐轴比对并打印。必需项 ('!') 不匹配 -> FAIL; 参考项 -> WARN; 基线没有的项只显示不
 * 判定; 基线要求但读不到的项 -> FAIL (必需) / WARN (参考)。
 * *dangerous_drift = 1 表示有 dangerous 项漂移, 调用者必须拒绝 S3/S4。 */
