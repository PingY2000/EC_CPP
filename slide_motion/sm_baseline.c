 ...
 *   与 2201h/2400h/2408h/2409h (行程当量, 决定脉冲与物理位移的换算)。
 *
...
static const sm_spec_t sm_spec_table[] = {
   ...
   /*
    * 这里原先还有一条 { 0x6041, ..., dangerous=1, "6041h 控制字" }。删掉它有两层理由,
    * 改名解决不了其中任何一层:
    *   1. 6041h 是**状态字**, 不是参数 (控制字是 6040h)。它随使能状态一直在变, 没有
    *      "期望值"可比 —— 静止时比出来的 FAIL 只说明"这会儿没使能", 什么也没证明。
    *   2. 它是 dangerous=1 的。于是 --dump-baseline 会把导出那一刻的瞬时状态字写进
    *      基线, 此后每次比对都必然 FAIL, 并且**拒绝 S3/S4** —— 一个自己造出来的死锁,
    *      而且正好卡在"按文档流程导出→审定→使用"这条路上。
    */

   /* ---- 行程当量: 决定"1 个脉冲 = 多少位移", 漂移即禁止动作 ----
    *
    * 2201h 是这一组的总开关, 它决定下面两套里哪一套说了算:
    *   2201h = 0 (出厂默认) -> 当量 = 2400h 细分 (本机实测 50000 脉冲/圈)
    *   2201h = 1            -> 当量 = 2408h/2409h 电子齿轮比
    * 两套差 50 倍。所以 2201h 一翻, 表里其它每一行都可以原封不动, 而"走 N 个脉冲 =
    * 走多远"整体变掉 —— 这正是 dangerous 要挡的那种漂移, 也正是当初漏掉它的代价。
    *
    * 2400h 与 2408h/2409h 谁生效是互斥的, 但四个都留在表里比: 生效的那个漂了要挡;
    * 不生效的那个漂了说明有人正在动配置, 同样该挡。
    *
    * 宽度用 U16 是因为这几个对象在实机上自报 2 字节 (见 ec_motor.h 的 em_rd_any 注释)。
    * dt 只决定符号性, 实际按驱动器自报的 psize 解释, 所以这里 U16/U32 对无符号值等价。
    */
...
/*
 * 表长必须装得进 sm_baseline_t::slot —— sm_base_load 是拿**表下标**去索引那个数组的,
 * 表一旦长过 SM_SPEC_COUNT 就是静默越界写。加对象时如果撞上这条, 顺手把 sm.h 里的
 * SM_SPEC_COUNT 调大即可。
 */
